...
var Root = (_temp2 = _class = function (_Component) {
  ...
  Root.prototype.componentDidMount = function componentDidMount() {
    ...
    if (isBrowserLike) {
      this.dispose = this.props.store.subscribe(function () {
        var state = (0, _utils.getLocalState)(_this2.props.store.getState()),
            changed = false;
        ...
        if (changed) {
          _this2.props.store.dispatch({ type: '$$local.flushed' });
        }
      });
    }
  };
